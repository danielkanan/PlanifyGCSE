...
export default function EditTopicsPage() {
  ...
  const getSubjectData = useCallback(() => {
    ...
    if (subject === 'mathematics' || subject === 'combined-science') {
      // Default to higher tier for now - this should be dynamic based on user selection
      return board.subjects.find(s => s.id === `${subject}-higher`);
    }
    return board.subjects.find(s => s.id === subject);
  }, [examBoard, subject]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.push(`/subjects/${examBoard}/${subject}`);
    } catch (error) {
